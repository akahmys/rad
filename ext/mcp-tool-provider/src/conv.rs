...
impl From<CoreRpcCommand> for wit::RasRpcCommand {
    fn from(cmd: CoreRpcCommand) -> Self {
        match cmd {
            CoreRpcCommand::FileRead { path } => wit::RasRpcCommand::FileRead(path.to_string_lossy().to_string()),
            ...
            CoreRpcCommand::SetStreamTimeoutPolicy { target, policy } => wit::RasRpcCommand::SetStreamTimeoutPolicy(wit::SetStreamTimeoutPolicyPayload {
                ...
                policy: match policy {
                    rad_models::TimeoutPolicy::Dynamic { heartbeat_timeout_ms, max_silent_wait_ms } => wit::TimeoutPolicy::Dynamic(wit::DynamicPolicy {
