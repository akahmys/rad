    ...
    if let Ok(ue) = serde_json::from_str::<ExtUnifiedError>(e) {
        match ue.level.as_str() {
            "L2" => {
                ...
                let _ = call_host(RasRpcCommand::CheckoutSnapshot { node_id: assistant_node_id.to_string() });
                format!("Tool call \"{tc_name}\" was not executed: {msg}")
            }
            "L3" => {
...
fn execute_pending_calls(assistant_node_id: &str, pending_calls: Vec<PendingToolCall>) -> Result<(), String> {
