...
use crate::tool::{Message, ToolCall, ToolCallFunction, ToolExecutionResult, execute_tool};
...
pub fn process_completed_tool_calls(pending: Vec<PendingToolCall>) -> Result<(), String> {
    for tc in pending {
        let raw_result = tc
            .result
            .unwrap_or_else(|| "No execution result.".to_string());
...
pub fn execute_and_collect_tools(pending_calls: Vec<PendingToolCall>) -> Result<(), String> {
    ...
    let state = state_guard
        .as_mut()
        .ok_or("State is None in execute_and_collect_tools")?;
...
pub fn extract_tool_calls(state: &mut OrchestratorState) -> (Vec<ToolCall>, Vec<PendingToolCall>) {
