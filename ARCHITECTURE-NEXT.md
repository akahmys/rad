...
現行コードベースの構造には拘束されない。これは漸進的リファクタリングではなく、
カーネルとモジュールSDKを新規に作り、既存のロジックを移植する計画である。
...
`net-open` は **要検討**。`wasi:http` の outgoing-handler で代替できる可能性があるが、
SSEストリーミングの扱いを確認していない。代替できるなら syscall は2つになる。
...
**これは移行ではなく再構築である。** 現行コードが漸進的に本設計になることはない。
ただし移植できるロジックは多い — 圧縮パイプライン、L3バックオフ、サーキットブレーカー、
DAG、スナップショット、PGIDグループkill、SSE解析。

| # | 内容 |
|---|---|
| 0 | **dialect構造体を現行コードに入れる** — 現行の `ext/llm-connector` 内で完結し、共有WITに触れない。単独でGemini/Azureが射程に入り、新設計にそのまま移植できる。**唯一、判断を待たずに着手できる項目** |
| 1 | `wit/kernel.wit` を確定させ、`rad-abi` と `rad-sdk` を作る |
| 2 | `rad-kernel` — wasmtime(async + epoch)、syscall、ルータ、スケジューラ(§3.6) |
| 3 | `context` モジュールを移植する(既存ロジックが最も素直に載る) |
| 4 | `llm-transport-openai` を移植し、実際にLLMが動く状態にする |
| 5 | `agent-loop` / `dag` / `ui-repl` を移植し、自立させる |
| 6 | `mcp-bridge` / `policy` / `skills`(§4.5 の3変更を同時に反映し、`CONFIG.md` §2.5 を更新) |
| 7 | `llm-transport-raw`(minijinja)を新規に作る |
| 8 | `templates/module-rust` と `cargo xtask new-module`、拡張作者向けドキュメント |

段階0以外は新規リポジトリまたは新規ディレクトリで進め、現行radを壊さずに並行させる。
段階5の時点で新旧が入れ替わる。
