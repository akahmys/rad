...
`net-open -> byte-stream` を定義していれば、WASI の版が変わってもカーネル内部の実装を
...
#### 3.6.9 ロック順序の不変条件(実測により追加)

カーネルは `deliver` の間そのモジュールのロックを保持する。つまり
**他モジュールを呼ぶモジュールは同時に2つのロックを持つ**。cycle check はこれを
見られない — スタックはスレッドローカルであり(§3.6.3)、2スレッドが1つずつ
ロックを取って互いのもう1つへ手を伸ばしても、どちらのスタックにも循環は現れない。

`tests/kernel_lock_order_tests.rs` で**実際にデッドロックすることを確認済み**。
epoch interruption も効かない — ゲストコードしかプリエンプトせず、両スレッドとも
ホスト呼び出しの中で `Mutex` を待っているためである。

したがって §3.6.1 の async スケジューラが入るまで、次を守る:

> **モジュールのロックを2つ以上同時に持ってよいのはイベントループスレッドだけである。**
> 他のスレッドは `post`(キューに触るだけ)はできるが、他モジュールへ転送する
> モジュールへ `call` してはならない。

これが「イベントは必ず `post`」(§3.6.2)を構造上の要請から**実測された要請**へ
格上げする。第三者のモジュール作者が知る必要がある規則でもある(§5.3)。
...
### 4.4.1 モジュールがツールを提供する規約

レジストリは1メソッドを1モジュールに割り当てる。したがって複数のツールプロバイダが
素の `tools.list` を名乗ることはできない — 2つ目の登録は起動時エラーになる。各
モジュールは他のメソッドと同じ `<module>.<method>` 規則に従って自分で名前空間を切り、

| メソッド | 引数 | 戻り値 |
|---|---|---|
| `<module>.tools.list` | `{}` | `{"tools": [ OpenAI function 形式 ]}` |
| `<module>.tools.call` | `{"name":…, "arguments":…}` | `{"content": "…"}` |

ホストがモジュールを走査して集約する(`src/kernel/tools.rs`)。`dispatch.call` が
「どのモジュールか知らずに呼ぶ」のと逆向きに見えるが、それが狙いである — **集約は
ホストの仕事であり、どのモジュールも他のモジュールの存在を知る必要がない。**

両メソッドを持たないモジュールは黙って飛ばされる。ツールを提供しないモジュール
(`context-tools`、そして第三者が書くモジュールの大半)が普通の状態である。

ツールの出力は文字列である。旧 `execute-tool` は `execution-handle` を返す型だった
ため、答えを持っているだけのプロバイダがそれを運ぶプロセスを捏造する必要があり、
`skill-tool-provider` は実際に `echo -n` を叩いていた(Markdownを読むだけの拡張が
bash実行権限を要求していた)。ホスト側 import に `execute-tool-text` を足して、
排水ループはホストへ移した(AWU 960)。
...
**③ ツール表現を「1ツール + 索引」に変える**(✅ AWU 962 で実施)
...
スキル10個で概算 4,700文字 → **実測 935バイト**(`ten_skills_cost_one_schema`)。
**本文が呼び出し時にのみ読まれる**という段階的開示は現行のまま維持される。

実装時に2点足した。`name` は説明文で列挙するだけでなく JSON-Schema の `enum` でも
制約する — 索引はモデルが言い換えうる散文だが、`enum` は実際に出力できる値を縛る。
またスキルが0個のときはツール自体を出さない(空の索引は、取れない選択肢に
スキーマを払わせた上で失敗しかしない呼び出しを誘う)。
...
- **モジュールに渡す環境変数** — カーネルのローダは `inherit_env()` している。
  置き換え対象の拡張ローダ(`src/wasm/loader.rs`)がそうしていたためで、移植で
  挙動を変えないという原則に従った結果である。ただしこれは**プロセスの環境変数
  一式(APIキーを含む)を全モジュールに配る**ことを意味する。

  §3.4 の論法(preopen が `.` と `$HOME` を丸ごと開けている以上、細かいマスクは
  芝居)はここにもある程度当てはまる — `~/.rad/config.json` が読める時点で
  キーは読める。それでも、絞る理由ができた時点(第三者モジュールを平常運転で
  読み込むようになった時点)で見直す対象として記録しておく。今絞らないのは、
  絞る基準がまだ無く、`PATH` を必要とする `mcp-bridge`(段階5)が控えているため。
...
（移行開始時点の状態。段階3〜6で `wit/context-tools.wit` と
`wit/connector/` は移植の完了とともに削除された。論拠は当時の実測であり、
現在の本数ではない。）
...
**段階9の決定: `dag` モジュールがグラフを所有し、自分で永続化する。** ホストの
記憶域への窓口にするだけの案(モジュールと呼ぶ価値が薄い)と、DAGをホストに
残す案は退けた。§3.6.6 が「トラップしたモジュールはStoreを破棄して再ロード」と
定めているため会話の消失が懸念されたが、**露出は測ると進行中のターン1つ分**である
— `save_session` は元々タスク完了ごとにしか走っておらず、拡張がターン途中で
クラッシュすれば今日でもそのターンは失われる。モジュールは**変更のたびに**保存する
ので厳密に改善であり、この懸念を段階のゲート条件にはしない。

**端末は出力側だけが移る。** 行の読み取りはブロックし、呼び出し元だけを
サスペンドするには §3.6.1 の async が要る。それが入るまで REPL ループはホストに残り、
`ui-repl` は半分だけのモジュールである。

### 9.4 各段階の不変条件

- **`wit/rad.wit` の既存関数の型は移行の最後まで変更しない。** 変更すれば残っている
  拡張が全部壊れる(§2.1)。ただし**トップレベル import の追加**は別で、§2.2 の実験が
  「既存コンポーネントに影響しない」ことを示している。AWU 960 で `execute-tool-text`
