...
fn test_spawn_and_pgid_isolation() {
    ...
    let mut proc = manager.spawn_bash_process("sleep 10").unwrap();
...
fn test_stdout_stderr_capture() {
    ...
    let mut proc = manager
        .spawn_bash_process("echo 'hello stdout'; echo 'hello stderr' >&2")
        .unwrap();
...
fn test_dynamic_timeout() {
    let manager = ProcessManager::new();
    let mut proc = manager.spawn_bash_process("sleep 5").unwrap();
...
fn test_manager_drop_kills_descendants() {
    ...
    {
        let manager = ProcessManager::new();
        let proc = manager.spawn_bash_process("sleep 100").unwrap();
        proc_pid = proc.pgid();
