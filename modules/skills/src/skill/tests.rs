use super::{parse_skill_md, skill_dirs, substitute_args};
...
#[test]
fn test_parse_skill_md_reads_a_skill_without_a_mode_line() {
...
/// `mode` is no longer parsed — `subagent` only ever returned "not
/// implemented", and subagents were dropped as a goal (ARCHITECTURE-NEXT.md
/// §1.2). An existing `SKILL.md` still carrying the line must load rather than
/// be rejected, which is what this now checks.
#[test]
fn a_legacy_mode_line_is_ignored_rather_than_rejected() {
    let content = "---\ndescription: Something.\nmode: subagent\n---\n\nBody.";
    let skill = parse_skill_md("s", content).unwrap();
    assert_eq!(skill.description, "Something.");
    assert_eq!(skill.body, "Body.");
