//! Ported from the `context-tools` extension unchanged except for how a
//! request is built and the call is made. Keeping the assertions identical is
//! the point: they are the evidence that swapping the caller over in AWU 957
...
#[test]
fn a_zero_budget_is_rejected_rather_than_emptying_the_history() {
    // Not present in the extension: it had no way to report this, so a caller
    // passing zero got an empty list back and found out much later.
    for (h, c) in [(Some(0), None), (None, Some(0))] {
        let request = OptimizeReq {
            messages: vec![msg("1", "user", "Hello")],
            max_history: h,
            max_content_chars: c,
        };
        assert!(optimize(request).is_err(), "h={h:?} c={c:?}");
    }
}
