//! Pure message-list compaction logic, ported unchanged from the
//! `context-tools` extension.
//!
//! It moved first because it was already the least entangled thing in the
//! tree: the original file opened by noting it had no `host_rpc` dependency,
//! so the port is a change of `Message`'s definition and nothing else. That
//! record came from WIT before; under opaque dispatch it is an ordinary serde
//! struct, which is the whole difference between the two architectures showing
//! up in one `use` line.
use crate::Message;
